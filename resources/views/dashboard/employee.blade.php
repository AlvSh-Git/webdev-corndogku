    ...
    function rebuildWindow(newWinStart, selectedISO) {
        ...
        days.forEach(d => {
            ...
            if (isFut) {
                ...
                datesHtml += `<button type="button" class="cal-date-btn" data-date="${dStr}" style="width:28px;height:28px;display:flex;align-items:center;justify-content:center;border-radius:50%;font-size:11px;font-weight:700;color:#fff;margin:auto;background-color:var(--color-primary);border:none;cursor:pointer;">${d.getDate()}</button>`;
            } else {
                const col = isToday ? 'var(--color-primary)' : 'var(--color-black)';
                const fw  = isToday ? '700' : '400';
                datesHtml += `<button type="button" class="cal-date-btn" data-date="${dStr}" style="width:28px;height:28px;display:flex;align-items:center;justify-content:center;border-radius:50%;font-size:11px;color:${col};font-weight:${fw};margin:auto;background:transparent;border:none;cursor:pointer;">${d.getDate()}</button>`;
            }
        });
